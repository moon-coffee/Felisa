...
| `MODERATION_SYNC` | （非同期） | `true` で投稿時に**同期チェック**（保存前に判定し、違反なら 400 で拒否）。既定は保存後すぐ応答してバックグラウンドで判定 |
...
Ollama が起動していない場合もサーバーは通常どおり動作します（判定不能＝「checking / pending」として投稿は通し、接続回復後の再スキャンで判定されます）。

判定はバックグラウンドで走るため、違反と判定されるまで投稿はタイムラインに表示されます（表示中でも削除は可能です）。判定結果は通知で届きます。
